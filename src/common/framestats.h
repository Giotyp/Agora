 ...
 public:
  ...
  inline size_t GetBeaconDlIdx(size_t symbol_id) const {
    ...
    if (type == SymbolType::kBeacon) {
      ...
      symbol_idx = GetDLCalSymbolIdx(symbol_id) + NumBeaconSyms();
    } else if (type == SymbolType::kControl) {
      symbol_idx =
          GetDLControlSymbolIdx(symbol_id) + NumBeaconSyms() + NumDLCalSyms();
    } else if (type == SymbolType::kDL) {
      symbol_idx =
          GetDLSymbolIdx(symbol_id) + NumDlBcastSyms() + NumDLCalSyms();
    } else {
